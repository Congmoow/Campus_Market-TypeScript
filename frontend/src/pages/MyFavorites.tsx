...
const MyFavorites: FC = () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const mapped: FavoriteItem[] = list.map((item: FavoriteWithProduct) => {
          ...
          const imageUrl = product.images?.[0]?.url || 'https://images.unsplash.com/photo-1545239351-1141bd82e8a6?auto=format&fit=crop&q=80&w=800';
  ...
  const handleRemove = async (productId: number) => {
    try {
      ...
      console.error('取消收藏失败', caughtError);
    }
  };
