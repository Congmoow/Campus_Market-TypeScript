...
const getCampusValue = (user?: User | CurrentUserLike | null): string | undefined => {
  ...
  const profile =
    'profile' in user && user.profile && typeof user.profile === 'object'
      ? (user.profile as { campus?: string })
      : undefined;
  const campus = 'campus' in user && typeof user.campus === 'string' ? user.campus : undefined;

  return profile?.campus || campus;
};
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          const sellerAvatar =
            getUserAvatarUrl(
              ...
              `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(
                String(
                  productDetail.sellerId || productDetail.seller?.studentId || productDetail.id,
                ),
              )}`,
            ) || '';
          ...
          setProduct({
            ...
            seller: getUserDisplayName(
              productDetail.seller,
              productDetail.seller?.studentId || '同学',
            ),
            sellerAvatar,
          });
        } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 relative overflow-hidden font-sans">
      ...
      <div className="relative pt-28 pb-32 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading && (
          <div className="text-center text-slate-400 text-sm py-20">正在加载下单商品信息...</div>
        )}
        {pageError && !loading && (
          <div className="text-center text-red-500 text-sm py-20">{pageError}</div>
        )}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 z-10">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">
                            在 {product.location} 约定地点，当面完成交易。
                          </p>
                        </div>
                      </div>
                      <div
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <p className="text-sm leading-relaxed font-medium">
                        ...
                        为了保障你的资金安全，请务必在{' '}
                        <span className="underline decoration-orange-300 underline-offset-2">
                          当面验货确认商品无误后
                        </span>{' '}
                  ...
                  >
                    ...
                    <div className="space-y-4 mb-8">
                      ...
                      <div className="flex justify-between text-slate-600">
                        <span>收货人</span>
                        <span className="font-medium text-slate-900">
                          {buyerContact?.name || '待补充'}
                        </span>
                      </div>
                      <div className="flex justify-between text-slate-600">
                        <span>联系电话</span>
                        <span className="font-medium text-slate-900">
                          {buyerContact?.phone || '待补充'}
                        </span>
                      </div>
