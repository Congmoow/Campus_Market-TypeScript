...
import type { ProductWithDetails } from '@campus-market/shared';
...
const ProductDetail: React.FC = () => {
  ...
  useEffect(() => {
    const loadDetail = async () => {
      ...
      try {
        ...
        if (res.success) {
          const rawProduct: ProductWithDetails = res.data;
          const images = rawProduct.images.length
            ? rawProduct.images.map((image) => image.url)
            : [
          ...
          const sellerAvatar =
            getUserAvatarUrl(
              ...
              `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(
                String(rawProduct.sellerId || rawProduct.seller?.studentId || rawProduct.id)
              )}`
            ) || '';
          ...
          setProduct({
            ...
            images,
            location: rawProduct.location || '校内',
            timeAgo: formatTimeFromString(rawProduct.createdAt),
            viewCount: rawProduct.viewCount ?? 0,
            seller: {
              id: rawProduct.sellerId,
              name: getUserDisplayName(rawProduct.seller, '同学'),
              major: rawProduct.seller?.profile?.major || '在校学生',
              credit: '信用良好',
              avatar: sellerAvatar,
            },
          });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Ignore view-count failures because they are non-blocking.
      }
    })();
  }, [id]);
  ...
  useEffect(() => {
    ...
    const loadFavoriteState = async () => {
      ...
      try {
        ...
        const favoriteIds = new Set((res.data || []).map((favorite) => favorite.product.id));
        setIsFavorite(favoriteIds.has(Number(id)));
      } catch {
        // Ignore favorite preload failures.
      }
    };
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      window.setTimeout(() => setFavoriteTip(''), 1500);
    } catch (caughtError) {
      console.error('收藏操作失败', caughtError);
      setIsFavorite(!next);
      setFavoriteTip('收藏操作失败，请稍后重试');
      window.setTimeout(() => setFavoriteTip(''), 2000);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="pt-32 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {error && !loading && <div className="text-center text-red-500 text-sm py-20">{error}</div>}
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4">
                {product.images.map((image, index) => (
                  <div
                    key={index}
                    className="aspect-square rounded-xl overflow-hidden bg-white shadow-sm border border-slate-100 cursor-pointer hover:ring-2 hover:ring-blue-500 transition-all opacity-80 hover:opacity-100"
                  >
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                <Link
                  to={product.seller.id ? `/user/${product.seller.id}` : '#'}
                  className="flex items-center gap-4 group"
                >
                  <img
                    src={product.seller.avatar}
                    alt=""
                    className="w-14 h-14 rounded-full bg-slate-100 ring-2 ring-slate-50 group-hover:ring-blue-200 transition-all"
                  />
                  <div>
