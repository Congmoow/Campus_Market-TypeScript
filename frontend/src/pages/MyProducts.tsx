...
import type { Category, ProductListItem, ProductStatus, ProductWithDetails, UpdateProductRequest } from '@campus-market/shared';
...
const MyProducts: FC = () => {
  ...
  const [filter, setFilter] = useState<'ALL' | 'ON_SALE' | 'SOLD'>('ALL');
  ...
  const [confirmNewStatus, setConfirmNewStatus] = useState<ProductStatus | null>(null);
  ...
  const loadCategories = async () => {
    try {
      ...
      console.error('加载分类失败', caughtError);
    }
  };
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      console.error('加载商品失败', caughtError);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'ON_SALE' || status === 'SOLD') return null;
    return (
  ...
  const handleToggleStatus = (product: ProductListItem) => {
    const newStatus: ProductStatus = product.status === 'ON_SALE' ? 'SOLD' : 'ON_SALE';
  ...
  const handleEdit = async (product: ProductListItem) => {
    ...
    try {
      ...
      console.error('加载商品详情失败', caughtError);
      alert('加载商品详情失败，请稍后重试');
    }
  };

  const handleSaveEdit = async (updatedData: UpdateProductRequest) => {
    ...
    try {
      ...
      console.error('更新商品失败', caughtError);
  ...
  const handleDelete = async (product: ProductListItem) => {
    ...
    try {
      ...
      console.error('删除商品失败', caughtError);
      alert('删除商品失败，请稍后重试');
    } finally {
      setDeletingId(null);
      setShowActionMenu(null);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-32">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <div className="w-40 h-40 absolute -top-24 left-56 z-10">
                <LazyLottie animationData={loaderCat} loop autoplay style={{ width: '100%', height: '100%' }} />
              </div>
            </div>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-8 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="flex p-1 bg-slate-100 rounded-xl">
            {(['ALL', 'ON_SALE', 'SOLD'] as const).map((status) => (
              <button
                key={status}
              ...
              >
                {status === 'ALL' ? '全部' : status === 'ON_SALE' ? '在售' : '已售出'}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((index) => (
              <div key={index} className="bg-white rounded-2xl h-80 animate-pulse shadow-sm border border-slate-100"></div>
            ))}
          </div>
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                    <span className="text-xl font-bold text-red-500 font-mono">¥{product.price}</span>
                        ...
                        {showActionMenu === product.id && (
                          ...
                          >
                            ...
                            <button
                              ...
                              className="w-full px-4 py-2 text-left text-sm hover:bg-blue-50 text-slate-700 flex items-center gap-2"
                            >
                              <Package size={14} /> {product.status === 'ON_SALE' ? '标记已售' : '重新上架'}
                            </button>
            ...
            >
              ...
              <div className="bg-slate-50 rounded-2xl px-4 py-3 text-left text-sm text-slate-600 mb-4">
                ...
                <div className="flex justify-between text-xs text-slate-500">
                  <span>
                    价格：<span className="font-semibold text-slate-900">¥{confirmingProduct.price}</span>
                  </span>
              ...
              <div className="flex justify-end gap-3 mt-4">
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (res.success) {
                        setProducts((prev) =>
                          prev.map((product) =>
                            product.id === confirmingProduct.id
                              ? { ...product, status: confirmNewStatus }
                              : product
                          )
                        );
                      ...
                      console.error('更新商品状态失败', caughtError);
                ...
                >
                  {confirmLoading ? '处理中...' : confirmNewStatus === 'SOLD' ? '标记已售' : '重新上架'}
                </button>
              </div>
            </motion.div>
          </motion.div>
